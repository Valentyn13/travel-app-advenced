...
const getUserBookings = createAsyncThunk<IBookingList,void>('user-bookings',async () =>{
    const bookings =await  bookingApi.getUserBookings()
    return bookings
})

const createBooking = createAsyncThunk<IBooking,CreateBookingDto>('create-booking',async (data) => {
    const booking = await bookingApi.createBooking(data)
    return booking
})


const cancelBooking = createAsyncThunk<cancelResponse,string>('cancel-booking', async (bookingId) => {
